...
partner_one_email = "sibandavensen@gmail.com"
partner_two_email = "nellynalediphiri@gmail.com"
assignment = "hog"
...
def free_bacon(opponent_score):
    ...
    return 1 + max(int(digit) for digit in str(opponent_score))
    if num_rolls == 0:
...
def is_swap(score, opponent_score):
    """Return True if the scores should be swapped."""
    return score == 2 * opponent_score or opponent_score == 2 * score
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        ...
        if who == 0:
            ...
            score += take_turn(num_dice, opponent_score, select_dice(score, opponent_score))
        # Player 1's turn
        elif who == 1:
            num_dice = strategy1(opponent_score, score)
            opponent_score += take_turn(num_dice, score, select_dice(opponent_score, score))
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for num_rolls in range(1, 11):
        # Assuming we want to calculate the average over a large number of trials
        average_score = sum(roll_dice(num_rolls, dice) for _ in range(1000)) / 1000
        print(f"{num_rolls} dice scores {average_score:.1f} on average")
...
def swap_strategy(score, opponent_score):
    ...
    if is_swap(new_score, opponent_score) and new_score < opponent_score:
        return 0
    # Check for harmful swap
    elif is_swap(new_score, opponent_score) and new_score > opponent_score:
        return BASELINE_NUM_ROLLS
    # Check if Free Bacon points are at least BACON_MARGIN
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # Check for beneficial or harmful swap
    if is_swap(new_score, opponent_score) and beneficial_swap(new_score, opponent_score):
        return 0
    elif is_swap(new_score, opponent_score) and not beneficial_swap(new_score, opponent_score):
        return BASELINE_NUM_ROLLS
    
    # Check for Free Bacon rule
    if bacon_points >= BACON_MARGIN:
        return 0
    
    # Play conservatively if close to winning
    if score >= 90:
        return 2  # Rolling fewer dice when close to winning
    
    # Default to baseline number of rolls
    return BASELINE_NUM_ROLLS
